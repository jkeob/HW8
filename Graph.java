...
public class Graph {
    ...
    public int findRoot() {
        ArrayList<Integer> keepTrackOfUsedNums = new ArrayList<>();

        // collect all vertices that appear in the rows
        for (int i = 0; i < adjListArr.length; i++) {
            for (int j = 0; j < adjListArr[i].size(); j++) {
                keepTrackOfUsedNums.add(adjListArr[i].get(j));
            }
        }

        int rootCountIfThere = 0;
        int rootIndexOf = -1;

        for (int i = 0; i < numVertices; i++) {
            if (!keepTrackOfUsedNums.contains(i)) {
                rootCountIfThere++;
                rootIndexOf = i;
            }
        }

        if (rootCountIfThere == 1) {
            return vertexValues.get(rootIndexOf);
        } else {
            return -1;
        }
    }
